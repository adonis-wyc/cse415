...
def handle_transition(action, new_state, r):
    ...
    sample = float("-inf")
    for a in ACTIONS:
        old_Q_value = Q_VALUES[(new_state, a)]
        sample = max(sample, old_Q_value)
    sample = r + GAMMA * sample
...
def choose_next_action(s, r, terminated=False):
    ...
    if not (s == INITIAL_STATE):
        ...
        sample = float("-inf")
        for a in ACTIONS:
            old_qval = Q_VALUES[(s, a)]
            sample = max(sample, old_qval)
        sample = r + GAMMA * sample
    ...
    if EPSILON > 0 or CUSTOM_EPSILON:
        ran = random.random()
        some_action = find_best_action(s, ACTIONS)
        if ran < EPSILON:
...
def find_best_action(s, A):
    qval = float("-inf")
    for a in A[0:-1]:
        if qval < Q_VALUES[(s, a)]:
            some_action = a
            qval = Q_VALUES[(s, a)]
    if qval == 0.0:
...
def extract_policy(S, A):
    ...
    for s in S:
        best_action = find_best_action(s, A)
        if is_valid_goal_state(s):
            Policy[s] = "Exit"
        elif s == Terminal_state:
            Policy[s] = None
