...
class State:
    ...
    def h_euclidean(self):
        ...
        for i in range(1, 9):
            (vi, vj) = self.find_location(i)
            (wi, wj) = (i // 3, i % 3)
            euclidean_distance += math.sqrt(math.pow(vi - wi, 2) + math.pow(vj - wj, 2))
    ...
    def h_manhattan(self):
        ...
        for i in range(1, 9):
            (vi, vj) = self.find_location(i)
            (wi, wj) = (i // 3, i % 3)
            manhattan_distance += math.fabs(vi - wi) + math.fabs(vj - wj)
        return manhattan_distance

    def h_custom(self):
        return (self.h_euclidean() + self.h_manhattan()) / 2.0
    ...
    def find_location(self, tile):
        '''Return the (vi, vj) coordinates of the tile.
        vi is the row index of the void, and vj is its column index.'''
        for i in range(3):
            for j in range(3):
                if self.b[i][j] == tile:
                    return (i, j)
        raise Exception("No tile" + tile + " location in state: " + str(self))
